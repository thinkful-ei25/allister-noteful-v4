...
describe.only('Noteful API - Notes', function () {
  ...
  describe('GET /api/notes', function () {
    ...
    it('should return correct search results for a title search', function () {
      ...
      const dbPromise = Note
        .find({$and : [{ $or: [{ title: re }, { content: re }] }, { userId: user.id }]})
        .sort({ updatedAt: 'desc' });
    ...
    it('should return an empty array for an incorrect query', function () {
      ...
      const re = new RegExp(searchTerm, 'i');
      const dbPromise = Note.find({$and : [{ $or: [{ title: re }, { content: re }] }, { userId: user.id }]}).sort({ updatedAt: 'desc' });
      const apiPromise = chai.request(app).get(`/api/notes?searchTerm=${searchTerm}`)
      .set('Authorization', `Bearer ${token}`);
      return Promise.all([dbPromise, apiPromise])
        .then(([data, res]) => {
  ...
  describe('PUT /api/notes/:id', function () {
    ...
    it('should unset a note folderId when provided a empty string', function () {
      ...
      return Note.findOne({$and : [{ folderId: { $exists: true } },{ userId: user.id }]} )
        .then((note) => {
          data = note;
          return chai.request(app).put(`/api/notes/${note.id}`)
          .set('Authorization', `Bearer ${token}`)
